...
*/
	if ($update == 'cancel') 
		mysql_query("UPDATE update_progress 
		SET update_status = 0,
	...
	if ($cfg['access_admin']) { 
		...
		if ($row["update_status"] <> 1 && mysql_num_rows($result)>0) {
...
/*	
	...
	$skipped_image	= mysql_num_rows(mysql_query('SELECT album_id FROM bitmap WHERE flag = 1 OR flag = 2'));
...
function playerProfile() {
	...
	$query = mysql_query('DESCRIBE session player_id');
	$default = mysql_fetch_assoc($query);
	$default = $default['Default'];
	$query = mysql_query('SELECT player_id
		FROM player
		WHERE player_id = ' . mysql_real_escape_string($default));
...
<?php
	$i=0;
	$query = mysql_query('SELECT player_name, player_type, player_id FROM player ORDER BY player_name');
...
function editPlayerProfile() {
	...
	if ($player_id == 0) {
		...
		mysql_query('INSERT INTO player (player_name) VALUES ("")');
		$player_id = mysql_insert_id($db);
	}
	else {
		// Edit configutaion
		...
		$query = mysql_query('SELECT player_name, player_type, player_host, player_port, player_pass, media_share FROM player WHERE player_id = ' . (int) $player_id);
...
function savePlayerProfile() {
	...
	mysql_query('UPDATE player SET
		player_name	= "' . mysql_real_escape_string($player_name) . '",
		player_type	= ' . (int) $player_type . ',
		player_host	= "' . mysql_real_escape_string($player_host) . '",
		player_port	= ' . (int) $player_port . ',
		player_pass	= "' . mysql_real_escape_string($player_pass) . '",
...
function setDefaultPlayerProfile() {
	...
	mysql_query('ALTER TABLE session
...
function deletePlayerProfile() {
	...
	mysql_query('DELETE FROM player WHERE player_id = ' . (int) $player_id);
}
...
function streamProfile() {
	...
	$query = mysql_query('DESCRIBE session stream_id');
	$default = mysql_fetch_assoc($query);
	$default = $default['Default'];
...
function downloadProfile() {
	...
	$query = mysql_query('DESCRIBE session download_id');
	$default = mysql_fetch_assoc($query);
	$default = $default['Default'];
	if ($default != -1 && isset($cfg['encode_extension'][$default]) == false) {
		$default = -1;
...
function skinProfile() {
	...
	// Get default skin
	$query = mysql_query('DESCRIBE session skin');
	$default = mysql_fetch_assoc($query);
	$default = $default['Default'];
?>
<table cellspacing="0" cellpadding="0" class="border">
...
function setSkinProfile() {
	...
	mysql_query('UPDATE session
		SET skin	= "' . mysql_real_escape_string($skin) . '"
		WHERE sid	= BINARY "' . mysql_real_escape_string($cfg['sid']) . '"');
}
...
function setDefaultSkinProfile() {
	...
	mysql_query('ALTER TABLE session
		CHANGE skin skin VARCHAR(255) NOT NULL DEFAULT "' . mysql_real_escape_string($skin) . '"');
}
