...
$query = mysql_query('SELECT artist, artist_alphabetic, album FROM album WHERE album_id = "' . mysql_real_escape_string($album_id) . '"');
...
$i = 0;
$query = mysql_query('SELECT title, artist, relative_file, track_id FROM track WHERE album_id = "' . mysql_real_escape_string($album_id). '" AND disc = ' . (int) $disc . ' ORDER BY relative_file');
...
$i = 0;
$query = mysql_query('SELECT relative_file, track_id FROM track WHERE album_id = "' . mysql_real_escape_string($album_id). '" AND disc = ' . (int) $disc . ' ORDER BY relative_file');
while ($track = mysql_fetch_assoc($query)) {
	$i++;
