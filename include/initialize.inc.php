...
define('NJB_HTTPS', (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != '') ? true : false);
...
if (function_exists('mysql_connect') == false)
	message(__FILE__, __LINE__, 'error', '[b]MYSQL not loaded[/b][list][*]Compile PHP with MYSQL support.[*]Or use a loadable module in the php.ini[/list]');
...
if (NJB_SCRIPT != 'message.php' && NJB_SCRIPT != 'cache.php')
	require_once(NJB_HOME_DIR . 'include/mysql.inc.php');

//  +------------------------------------------------------------------------+
//  | Check and set default favorite playlist								 |
//  +------------------------------------------------------------------------+
...
function authenticate($access, $cache = false, $validate_sign = false, $disable_counter = false) {
	...
	$query			= mysql_query('SELECT logged_in, user_id, idle_time, ip, user_agent, sign, seed, skin,
...
//  | Login                                                                  |
//  +------------------------------------------------------------------------+
	$query		= mysql_query('SELECT username FROM user WHERE username = "' . mysql_real_escape_string($cfg['anonymous_user']) . '"');
	$user		= mysql_fetch_assoc($query);
...
function setSkin($skin) {
	...
	if (file_exists(NJB_HOME_DIR . 'skin/' . $session['Default'] . '/styles.css'))	{
		if ($skin == '') {
			mysql_query('UPDATE session
				SET skin	= "' . mysql_real_escape_string($session['Default']) . '"
				WHERE sid	= BINARY "' . mysql_real_escape_string($sid) . '"');
		}
	...
	mysql_query('ALTER TABLE session CHANGE skin skin VARCHAR(255) NOT NULL DEFAULT "' . mysql_real_escape_string($cfg['skin']) . '"');
	return true;
}
...
function checkDefaultFavorites() {
	...
	$query = @mysql_query("SELECT * FROM favorite WHERE name = '" . mysql_real_escape_string($cfg['favorite_name']) . "' AND comment = '" . mysql_real_escape_string($cfg['favorite_comment']) . "' LIMIT 1");
	
	if (@mysql_num_rows($query) == 0) {
		@mysql_query("INSERT INTO favorite (name, comment, stream) 
		VALUES (
		'" . mysql_real_escape_string($cfg['favorite_name']) . "',
		'" . mysql_real_escape_string($cfg['favorite_comment']) . "',
		'0'
		)");
		$cfg['favorite_id'] = @mysql_insert_id($db);
	}
	else {
