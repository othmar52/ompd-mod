...
global $cfg, $db;
	...
	$sid = $_GET['sid'];
	mysql_query ('UPDATE session SET player_id="' . $player_id . '" WHERE
	sid="' . $sid . '"');
	$query = mysql_query('SELECT player_name, player_host, player_port FROM player WHERE player_id="' . $player_id . '"');
	$playerDB = mysql_fetch_assoc($query);
